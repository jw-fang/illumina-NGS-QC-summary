...
def start():
    ...
    return selected_sum,selected_lane,selected_index

selected_sum,selected_lane,selected_index=start()
    
def parse(location):
    
    summary=ic.summary(location,columns=selected_sum)
    df=pd.DataFrame(summary)
    
    summary_lane=ic.summary(location,"Lane",columns=selected_lane)
    ...
    if len(index)!=0:
        ...
        summary_index=ic.index_summary(location,columns=selected_index)
...
def export ():
    ...
    for path in paths:
        ...
        counter+=1
        print(f"{run_name} has been processed {counter}/{len(paths)}")
